...
object Altimeter {
  // Send to the Altimeter to inform it about
...
class Altimeter extends Actor with ActorLogging { this: EventSource =>
  ...
  // As time passes, we need to change the altitude baed on
  // the time pased. The lastTick allows us to figure out
  ...
  def altimeterReceive: Receive = {
    ...
    case RateChange(amount) =>
      // Truncate the range 'amount' to [-1, 1]
      // before multiplying
